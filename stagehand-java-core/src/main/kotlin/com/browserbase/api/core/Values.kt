...
sealed class JsonField<out T : Any> {
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw StagehandInvalidDataException("`$name` is not set")
            is JsonNull -> throw StagehandInvalidDataException("`$name` is null")
            else -> throw StagehandInvalidDataException("`$name` is invalid, received $this")
        }
