...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { BrowserbaseInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw BrowserbaseInvalidDataException("`$name` is not set")
            is JsonNull -> throw BrowserbaseInvalidDataException("`$name` is null")
            else -> throw BrowserbaseInvalidDataException("`$name` is invalid, received $this")
        }
